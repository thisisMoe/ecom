...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a class="btn btn-primary" href="{{ route('admin.mainCategories') }}">Main Categories</a>
    </div>
                    ...
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <div>
                            <h6 class="m-0 mb-2 font-weight-bold text-primary">
                                <a href="{{ route('admin.category', ['mainCat' => $mainCategory->id, 'id' => $cat->id]) }}">{!! $cat->name !!}</a>
                            </h6>
                ...
                <div class="modal fade" id="arabicNameModal-{{ $cat->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModal" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('admin.catArabicNaming', ['id'=>$cat->id]) }}" method="POST">
                                @csrf
                                <div class="modal-body">
                                    <input type="text" name="arabicName" value="{{ $cat->arabicName }}">
                                </div>
